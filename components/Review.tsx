...
const CustomerReviews: React.FC = () => {
    return (
        <div className="review-card py-8">
            <h2 className="text-3xl font-bold text-center mb-6 text-gradient">Customer Reviews</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
                {reviews.map((review, index) => (
                    <div
                        key={index}
                        className="review-card flex flex-col items-center text-gradient p-6 rounded-lg shadow-lg text-center hover:shadow-2xl transition-shadow duration-300 mx-auto"
                    >
                        <Image
                            src={review.avatar}
                            alt={review.name}
