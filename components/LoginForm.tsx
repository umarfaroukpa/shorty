...
const LoginForm = ({ onSwitchToSignup, activeForm }: LoginFormProps) => {
    ...
    return (
        <div className={`w-full max-w-xs mx-auto p-4 md:p-6 bg-white rounded-lg shadow-sm transition-all duration-500 ${activeForm === 'login' ? 'h-full text-base md:text-lg' : 'h-auto text-xs md:text-sm'}`}>
            ...
            >
                {showLoginFields ? (
                    ...
                    <><i className="fa-solid fa-right-to-bracket mr-1"></i> Login With Email</>
                )}
            </button>
            ...
            {showLoginFields && (
                <form onSubmit={handleSubmit} className="mt-3 space-y-2 md:space-y-3">
                    ...
                    >
                        <i className="fa-solid fa-sign-in-alt mr-1"></i> Sign In
                    </button>
                </form>
            )}
            
            <div className="mt-3 text-center text-xs md:text-sm">
                <button onClick={() => setShowForgotPassword(true)} className="text-blue-500 hover:underline">
                    Forgot Password?
                </button>
            </div>
            ...
            <Modal show={showForgotPassword} onClose={handleCloseModal}>
                ...
                {resetSent ? (
                    ...
                    <>
                        <p className="text-sm mb-2">Enter your email address to reset your password:</p>
                        <input
                            type="email"
                        ...
                        />
                        <button
                            className="w-full p-2 bg-blue-500 text-white rounded-md hover:bg-blue-700 mt-4 text-sm"
